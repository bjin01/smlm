...
def get_patches_by_advisory_type(client, key, systemname, systemid, advisory_type):
    ...
    if "security" in str(advisory_type).lower():
        internal_type = "security_patches"
    
    if "bug" in str(advisory_type).lower():
        internal_type = "bugfix_patches"
    
    if "enhancement" in str(advisory_type).lower():
        internal_type = "enhancement_patches"
...
def write_html(suma_config_file, output_html_file, pre_html_file, post_html_file, final_result, groups=[]):
    ...
    rows = {
        ...
        "col6": {"key_name": "security_patches", "column_name": "Security Updates"},
        "col7": {"key_name": "bugfix_patches", "column_name": "Bugfix Updates"},
        "col8": {"key_name": "enhancement_patches", "column_name": "Enhancement Updates"},
            ...
            for row in final_result:
                ...
                for key in rows.values():
                    ...
                    if k_name in row.keys():
                        ...
                        elif k_name in ["security_patches", "bugfix_patches"] and row.get(k_name, 0) > color_threshold:
                            param_file.write(f"<td style='color:red;'>{row.get(k_name, '')}</td>")
                        else:
...
def write_csv(suma_config_file, output_csv_file, final_result):
    ...
    try:
        with open(output_csv_file, mode='w', newline='', encoding='utf-8') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=[row["column_name"] for row in rows.values()])
            writer.writeheader()
            for row in final_result:
                writer.writerow({rows[key]["column_name"]: str(row.get(rows[key]["key_name"], "")).replace("\n", " ").replace("\r", " ") for key in rows})
        log.info(f"CSV report written to {output_csv_file}")
    except Exception as e:
...
def run_all(suma_config_file, output_csv_file="report.csv", **kwargs):
    final_result = []
    suma_config = _get_suma_configuration(suma_config_file)
    server = suma_config["servername"]
    ret = dict()
    ...
    if len(listSystems_result) > 0:
        for system in listSystems_result:
            if isinstance(system, dict):
                ...
                #print("patches_by_advisory_type {}".format(patches_by_advisory_type))
                system_dict.update(patches_by_advisory_type)

                final_result.append(system_dict)
    
    write_csv(suma_config_file, output_csv_file, final_result)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output-csv-file", default="report.csv", help="Path to output csv report")
    ...
    if args.groups and len(args.groups) > 0:
        run_by_group(args.config, args.output_csv_file, args.groups, **kwargs)
    else:
        run_all(args.config, output_csv_file=args.output_csv_file, **kwargs)
