...
class BaseGate(metaclass=abc.ABCMeta):
    ...
    def get_scenario_template(self, target: str = 'sqlplus', login: typing.Optional[str] = None) -> str:
        ...
        if target in ['sqlplus', 'rman']:
            ...
            #scenario.append(("cat - << EOF | " + executable + " -h db -t --pset footer=off " + self.config.get('db_name', '')).strip())
            scenario.append((
            "cat - << EOF | PGPASSWORD='{0}' {1} -h db -U {2} -t --pset footer=off {3}"
            ).format(
                self.config.get('db_password', ''),
                executable,
                self.config.get('db_user', 'spacewalk'),
    ...
    def call_scenario(self, scenario: str, target: str = 'sqlplus',
        ...
        return self.syscall("su", "-", user, "-c", "/bin/bash", input=template)
    ...
    def check_sudo(self, uid: str) -> None:
        ...
        print(f"uid is: {uid}")
        stdout, stderr = self.syscall("su", "-", uid, "-c", "/bin/bash")
        #if stdout or stderr:
