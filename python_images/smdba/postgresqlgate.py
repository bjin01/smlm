...
class PgSQLGate(BaseGate):
    ...
    def _get_rhn_config(self) -> None:
        ...
        """
        self.rhn_config = {}
        self.rhn_config_file = "/etc/rhn/rhn.conf"

        if os.path.exists(self.rhn_config_file):
            with open(self.rhn_config_file, encoding="utf-8") as f:
                rhn_cfg = f.readlines()
        else:
            raise Exception("Cannot open configuration file: " + self.rhn_config_file + "\n" + "Use sudo, perhaps?")
        ...
        for line in rhn_cfg:
            try:
                ...
                if key.startswith("db_"):
                    ...
                    self.rhn_config[key] = value.split("/")[-1] if (key == "db_name") and value.startswith("//") else value
            except Exception:
                pass
        print(f"show self.rhn_config {self.rhn_config}")
    ...
    def _get_pg_data(self) -> None:
        ...
        for line in open(self.config_file).readlines():
            #print(f"do we get here in line in open...{line}")
    ...
    def _get_pg_config(self) -> None:
        ...
        """
        stdout, stderr = self.syscall("su", "-", "postgres", "-c", "/bin/bash",
    ...
    def do_db_start(self, **args: str) -> None:  # pylint: disable=W0613
        ...
        if self._with_systemd:
            ...
            result = os.system("su - postgres -c /usr/bin/pg_ctl start -s -w -p /usr/bin/postgres -D %s -o %s 2>&1>/dev/null"
    ...
    def do_db_stop(self, **args: str) -> None:  # pylint: disable=W0613
        ...
        if self._with_systemd:
            ...
            # Container might not have systemd
            result = os.system("su - postgres -c /usr/bin/pg_ctl stop -s -D %s -m fast"
    ...
    def do_space_reclaim(self, **args: str) -> None:  # pylint: disable=W0613
        ...
        for msg, operation in operations:
            ...
            _, stderr = self.syscall("su", "-", "postgres", "-c", "/bin/bash",
    ...
    def _perform_enable_backups(self, **args: str) -> None:
        ...
        if enable == 'on':
            ...
            if not os.path.exists(backup_dir):
                os.system('su - postgres -c /bin/mkdir -p -m 0700 %s' % backup_dir)
            ...
            cwd = os.getcwd()
            os.chdir(self.config.get('pcnf_data_directory', '/var/lib/pgsql'))
            os.system('su - postgres -c "PGPASSWORD={0} PGSSLMODE=disable /usr/bin/pg_basebackup -h {1} -U {2} -D {3}/ -Ft -c fast -X fetch -v -P -z"'.
                      format(self.rhn_config.get("db_password"), 
                             self.rhn_config.get("db_host"), 
                             self.rhn_config.get("db_user"),
                             b_dir_temp))
            os.chdir(cwd)
